...
import { PositionSpecificAnalysis } from '../services/positionSpecificAnalyzer';

interface PositionSpecificAnalysisCardProps {
  analysis: PositionSpecificAnalysis;
  className?: string;
}

export const PositionSpecificAnalysisCard: React.FC<PositionSpecificAnalysisCardProps> = ({
  analysis,
  className = ""
}) => {
  ...
  return (
    <div className={`bg-white border border-gray-200 rounded-3xl p-8 shadow-2xl ${className}`}>
      ...
      <div className="flex items-center mb-8">
        ...
        <div>
          <h3 className="text-3xl font-bold text-black">{analysis.positionTitle} Resume Analysis</h3>
          <p className="text-gray-600 text-lg">Professional assessment for {analysis.positionTitle.toLowerCase()} roles</p>
        </div>
      </div>
      ...
      <div className="mb-8">
        <h4 className="text-xl font-bold text-black mb-6 flex items-center">
          ...
          Technical Skills Assessment for {analysis.positionTitle}
        </h4>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Object.entries(analysis.technicalSkillsBreakdown).map(([skill, data]) => {
            const IconComponent = getIconForCategory(skill);
            return (
              <motion.div
                key={skill}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        ...
        <div className="bg-blue-50 rounded-2xl p-6 border border-blue-200">
          ...
          {analysis.experienceRelevance.relevantExperience.length > 0 && (
            <div className="mb-3">
              <p className="text-sm font-semibold text-green-700 mb-1">✓ {analysis.positionTitle} Experience:</p>
              <p className="text-sm text-green-600">{analysis.experienceRelevance.relevantExperience.join(', ')}</p>
            </div>
          )}
          {analysis.experienceRelevance.transferableExperience.length > 0 && (
            <div className="mb-3">
              <p className="text-sm font-semibold text-blue-700 mb-1">~ Transferable Experience:</p>
              <p className="text-sm text-blue-600">{analysis.experienceRelevance.transferableExperience.slice(0, 3).join(', ')}</p>
            </div>
          )}
      ...
      {/* Industry Alignment & ATS Optimization */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        {/* Industry Alignment */}
        <div className="bg-green-50 rounded-2xl p-6 border border-green-200">
          <h4 className="text-lg font-bold text-green-800 mb-4 flex items-center">
            <Target className="w-5 h-5 mr-2" />
            Industry Alignment ({analysis.industryAlignment.alignmentScore}/10)
          </h4>
          {analysis.industryAlignment.keywords.length > 0 && (
            <div className="mb-3">
              <p className="text-sm font-semibold text-green-700 mb-1">✓ Industry Keywords:</p>
              <p className="text-sm text-green-600">{analysis.industryAlignment.keywords.slice(0, 3).join(', ')}</p>
            </div>
          )}
          {analysis.industryAlignment.certifications.length > 0 && (
            <div className="mb-3">
              <p className="text-sm font-semibold text-blue-700 mb-1">~ Certifications:</p>
              <p className="text-sm text-blue-600">{analysis.industryAlignment.certifications.join(', ')}</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-orange-50 rounded-2xl p-6 border border-orange-200">
          <h4 className="text-lg font-bold text-orange-800 mb-4 flex items-center">
            ...
            ATS Optimization ({analysis.atsOptimization.score}/10)
          </h4>
          {analysis.atsOptimization.missingKeywords.length > 0 && (
            <div className="mb-3">
              <p className="text-sm font-semibold text-red-700 mb-1">✗ Missing Keywords:</p>
              <p className="text-sm text-red-600">{analysis.atsOptimization.missingKeywords.slice(0, 3).join(', ')}</p>
            </div>
          )}
          {analysis.atsOptimization.suggestions.length > 0 && (
            <div>
              <p className="text-sm font-semibold text-orange-700 mb-1">💡 Suggestions:</p>
              <ul className="text-sm text-orange-600">
                {analysis.atsOptimization.suggestions.slice(0, 2).map((suggestion, index) => (
      ...
      {analysis.keyStrengths.length > 0 && (
        <div className="mb-8">
          <h4 className="text-lg font-bold text-black mb-4 flex items-center">
            ...
            Key Strengths for {analysis.positionTitle}
          </h4>
      ...
      <div className="mb-8">
        <h4 className="text-lg font-bold text-black mb-4 flex items-center">
          ...
          Major Gaps for {analysis.positionTitle}
        </h4>
        <div className="space-y-3">
      ...
      {analysis.irrelevantElements.length > 0 && (
        <div className="mb-8">
          <h4 className="text-lg font-bold text-black mb-4 flex items-center">
            ...
            Irrelevant Elements for {analysis.positionTitle}
          </h4>
      ...
      <div className="mb-8">
        <h4 className="text-lg font-bold text-black mb-4 flex items-center">
          ...
          Improvement Recommendations for {analysis.positionTitle}
        </h4>
        <div className="space-y-3">
      ...
      <div className="bg-gradient-to-r from-gray-800 to-black rounded-2xl p-6 text-white">
        <h4 className="text-xl font-bold mb-4 flex items-center">
          ...
          Honest Job Readiness Assessment for {analysis.positionTitle}
        </h4>
